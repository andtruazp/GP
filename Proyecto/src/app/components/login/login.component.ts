...
export class LoginComponent implements OnInit {
  ...
  login() {
    ...
    const resetTimer = () => {
      ...
      timeout = setTimeout(() => {
        // Limpiar sessionStorage y redirigir al inicio de sesión
        sessionStorage.removeItem('userData');
        this.router.navigate(['/login']); 
      }, 60000); // 1 minuto en milisegundos
    };
  
    // Agregar detección de eventos de mouse y teclado para reiniciar el temporizador
    ...
    this.http.get('http://localhost:3002/login', { params }).subscribe(
      (response: any) => {
        if (response.message === 'Inicio de sesión exitoso') {
          ...
          resetTimer(); // Reiniciar temporizador después de iniciar sesión exitosamente
        } else {
